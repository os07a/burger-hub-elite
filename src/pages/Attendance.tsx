...
const todayRecords = [
  { initials: "يو", name: "يونس", shift: "10:00 ص — 6:00 م", checkIn: "9:58 ص", checkInClass: "text-success", checkOut: "—", hours: "7.5 س", diff: "مبكر 2 دقيقة", diffClass: "text-success", status: "حاضر", statusVariant: "success" as const },
  { initials: "شي", name: "شيمول", shift: "9:00 ص — 5:00 م", checkIn: "9:02 ص", checkInClass: "text-success", checkOut: "—", hours: "8.5 س", diff: "في الوقت", diffClass: "text-success", status: "حاضر", statusVariant: "success" as const },
  { initials: "مي", name: "ميراج", shift: "2:00 م — 10:00 م", checkIn: "2:22 م", checkInClass: "text-warning", checkOut: "—", hours: "5.5 س", diff: "تأخر 22 دقيقة", diffClass: "text-warning", status: "تأخر", statusVariant: "warning" as const },
  { initials: "عم", name: "موظف 4", shift: "12:00 م — 8:00 م", checkIn: "—", checkInClass: "text-gray-light", checkOut: "—", hours: "0 س", diff: "لم يحضر", diffClass: "text-danger", status: "غائب", statusVariant: "danger" as const, absent: true, avatarBg: "bg-gray-light" },
];
...
const weekData: { initials: string; name: string; days: DotType[]; total: string; avatarBg?: string }[] = [
  ...
  { initials: "عم", name: "موظف 4", days: ["abs", "ok", "ok", "abs", "ok", "late", "off"], total: "4 / 6", avatarBg: "bg-gray-light" },
];
...
const Attendance = () => (
  <div>
    ...
    <div className="grid grid-cols-4 gap-3 mb-5">
      <MetricCard label="حاضرون" value="7" sub="من أصل 9" subColor="success" />
      <MetricCard label="تأخر" value="1" sub="ميراج — 22 دقيقة" subColor="warning" />
      <MetricCard label="غائب" value="1" sub="موظف 4" subColor="danger" />
      <MetricCard label="ساعات العمل اليوم" value="47.5" sub="ساعة إجمالي" />
    </div>
    ...
    <div className="bg-surface border border-border rounded-lg p-4 border-r-[3px] border-r-primary mb-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse min-w-[600px]">
          ...
          <tbody>
            {todayRecords.map((r) => (
              <tr key={r.name} className={`${r.absent ? "opacity-60" : ""} hover:bg-background/50`}>
                <td className="px-2.5 py-2.5 border-b border-border">
                  <div className="flex items-center gap-2">
                    <div className={`w-7 h-7 rounded-full ${r.avatarBg || "bg-primary"} flex items-center justify-center text-[11px] font-bold text-primary-foreground`}>{r.initials}</div>
        ...
        {weekData.map((emp) => (
          <div key={emp.name} className="grid bg-surface items-center" style={{ gridTemplateColumns: "160px repeat(7, 1fr) 70px" }}>
            <div className="px-2.5 py-2.5">
              <div className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-full ${emp.avatarBg || "bg-primary"} flex items-center justify-center text-[10px] font-bold text-primary-foreground`}>{emp.initials}</div>
